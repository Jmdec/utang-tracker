...
export type Tab = 'home' | 'people' | 'history' | 'expenses' | 'wallet' | 'reports' | 'settings' | 'goals';
...
export default function Home() {
  ...
  // Show action buttons only on relevant tabs
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden', position: 'relative' }}>
        ...
        <main style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', paddingBottom: isMobile ? '80px' : '20px' }}>
            {renderPage()}
          </div>
        </main>
      </div>
      ...
      <AddDebtModal    isOpen={showAddDebt}    onClose={() => setShowAddDebt(false)} />
    </div>
  );
}